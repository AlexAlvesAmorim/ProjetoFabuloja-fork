...
test.describe('Products - Public Catalog', () => {
  ...
  test('should track product view event', async ({ page }) => {
    ...
    const requestPromise = page.waitForResponse(
      response =>
        response.url().includes('/api/lead-events') && response.request().method() === 'POST'
    );
  ...
  test('should track buy click event', async ({ page }) => {
    const requestPromise = page.waitForResponse(
      response =>
        response.url().includes('/api/lead-events') &&
  ...
  test('should redirect to WhatsApp on WhatsApp button click', async ({ page }) => {
    const [popup] = await Promise.all([
      ...
      page
        .locator('[id^="secao-"] >> .group')
        .first()
        .click()
        .then(() => page.click('button:has-text("Comprar pelo WhatsApp")')),
    ]);
